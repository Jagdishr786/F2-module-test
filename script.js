...
//assinging jason data to variable to conver later into object
...
// unpacking the jason data by using parse method to convert it into object file 
let data = JSON.parse(mockData);

// salecting the input and button by DOM and assigning it to the new variable  
let searchInput = document.getElementById("myInput");

let searchBttn = document.getElementById("searchbtn");


//as to add the functionality to all the sorintg buttons assigned them to new varibals to add event listenr later on 

let Students = document.getElementById("students");
let sortbyAtoZ = document.getElementById("sortAtoZ");
let sortbyZtoA = document.getElementById("sortZtoA");
let sortbyMarks = document.getElementById("sortMarks");
let sortbyPassing = document.getElementById("sortPassing");
let sortbyClass = document.getElementById("sortClass");
let sortbyGender = document.getElementById("sortGender");


// adding the funvtion to assignd varibale one by one 
sortbyMarks.addEventListener("click", sortingByMarks);

sortbyClass.addEventListener("click", sortingByClass);

sortbyPassing.addEventListener("click", sortingByPassing);

searchBttn.addEventListener("click", searching);

sortbyAtoZ.addEventListener("click", sortingByAZ);

sortbyZtoA.addEventListener("click", sortingByZA);

sortbyPassing.addEventListener("click", sortingByPassing);

sortbyGender.addEventListener("click", sortingByGender);


// using .map to print the jason data on html file
...
function loadData() {
    ...
    data.map((item) => {
        ...
        lItem.innerHTML = `
   <td> ${item.id} </td>
    <td> <img_src="${item.img_src}</td> <!-- after salecting the img_src it was not able to fetch img hence failed to log it on window -->
    ${item.first_name}
    ${item.last_name} </td>
    <td> ${item.gender} </td>
    <td> ${item.class} </td>
...
// calling function after defining it to log the data on to window
loadData();



// usinf filter method and writing function foe searching but was not able to make work 
// there might be jason data isuue
function searching() {
    let s = searchInput.value;
    let matchedInput = data.filter((item) => {
        return item.first_name.toLowerCase().includes(s);
    })
    console.log(matchedInput);
    loadData(matchedInput);
}



// usinf sort function on object in arry to make sort by alphabetically from A TO Z
function sortingByAZ() {
    let higestalpha = data.sort((a, b) => {
    ...
    loadData(sortingByAZ)
}


// usinf sort function on object in arry to make sort by alphabetically from Z TO A
function sortingByZA() {
    let lowestalpha = data.sort((a, b) => {
        if (a.first_name.toLowerCase() < b.first_name.toLowerCase())
            return 1;
        else if (a.first_name.toLowerCase() > b.first_name.toLowerCase())
            return -1;
    ...
    loadData(sortingByZA);

}

// as mention above using sort to log jason data into asending marks 

function sortingByMarks() {
    let higestMarks = data.sort((a, b) => {
        if (a.marks < b.marks)
            return 1;
    ...
    loadData(sortingByMarks);

}

// tying to sort and log the students who are passed 
// was not able to hide the data of student who failed 
function sortingByPassing() {
    let resultBe = data.sort((a, b) => {
        if (a.passing == true)
            return -1;
    })

    loadData(sortingByPassing)
}

// used sort again to sort jason data by class in assending order 
function sortingByClass() {
    let byClass = data.sort((a, b) => {
        if (a.class < b.class)
            return 1;
    ...
    loadData(sortingByClass);

}

// was not able to figure out the sorting by gender
function sortingByGender() {
    let tgender = data.sort((a) => {
        if (a.gender === "Male")
            return 1;
    })

    loadData(sortingByGender);
